...
export default MyComponent;`}</Paragraph>
      ...
      <List
        ...
        bordered
        dataSource={ListDataPureFunction}
        renderItem={(item) => (
          <List.Item>
            <Text mark>😸</Text> {item}
          </List.Item>
  ...
  return <h1>Hello, {name}!</h1>;
      ...
      <Paragraph>В этом примере компонент Greeting принимает проп name и отображает приветствие. Он не хранит состояние и не изменяется с течением времени.</Paragraph>

      <Title level={5}>Stateful компоненты </Title>
      <Paragraph>
      Stateful <Text mark>компоненты могут управлять состоянием</Text> и изменять свое поведение в зависимости от событий. Они используют внутреннее состояние, чтобы отслеживать изменения и обновлять UI в ответ на действия пользователя или другие события.
      С появлением хуков в React 16.8 функциональные компоненты могут также управлять состоянием и эффектами. Теперь можно создавать stateful функциональные компоненты с помощью <Text strong type='danger'>хуков useState и useEffect.</Text>
      </Paragraph>
      <pre>
...
const Counter = () => {
  ...
  return (
    <div>
      ...
      <Paragraph>⚠️
      Stateless компоненты просто рендерят UI, в то время как stateful компоненты имеют внутреннее состояние, которое позволяет им изменять своё поведение в ответ на события.</Paragraph>
    </>
  );
};
